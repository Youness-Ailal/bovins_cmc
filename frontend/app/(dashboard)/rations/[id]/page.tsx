...
import type { Ration, StockArticle, Parcelle } from "@/lib/types";
...
export default function EditRationPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  useEffect(() => {
    if (!ration) return;
    setNom(ration.nom);
    setPhase(ration.phase || "Croissance");
    setCible(ration.cible || "");
    setIngredients(
      (ration.ingredients ?? []).map((ing, idx) => ({
        id: String(idx + 1),
        articleId: ing.article ?? "",
        nom: ing.nom,
        quantite: String(ing.quantite),
        unite: ing.unite,
        prixUnit: ing.prixUnitaire,
      }))
    );
  }, [ration]);
    ...
    <div className="flex flex-1 flex-col overflow-hidden bg-surface">
      <header className="flex h-14 shrink-0 items-center justify-between border-b border-border-light bg-card px-7">
        <div className="flex items-center gap-1.5">
          ...
          <span className="font-dm-sans text-[15px] font-semibold text-label">{ration?.nom ?? "Modifier la ration"}</span>
        </div>
        <div className="flex items-center gap-2">
          <Link
            href={`/rations/distribution?ration=${id}`}
          ...
          <button
            type="submit" form="ration-edit-form" disabled={submitting}
      ...
      <form id="ration-edit-form" onSubmit={handleSubmit} noValidate className="flex flex-1 flex-col gap-4 overflow-auto p-6">
        {/* General info */}
          ...
          {ingredients.length === 0 ? (
            ...
            <div className="mt-4">
              ...
              {ingredients.map((ing) => {
                ...
                const artInStock = stockList.find((a) => a.id === ing.articleId);
                return (
                  <div key={ing.id} className="grid grid-cols-[1fr_100px_70px_100px_90px_28px] items-center gap-2 border-b border-border-light py-2.5 last:border-b-0">
                    ...
                    >
                      {!artInStock && ing.nom && <option value={ing.articleId}>{ing.nom}</option>}
                      {stockList.map((a) => <option key={a.id} value={a.id}>{a.designation}</option>)}
                    </select>
                    <div className="flex h-9 items-center rounded-[6px] border border-border-light bg-surface px-2">
                      <input
